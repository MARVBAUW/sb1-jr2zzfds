...
const formatNumber = (value: number, options: { style?: string; currency?: string; minimumFractionDigits?: number; maximumFractionDigits?: number } = {}) => {
            ...
            <dd className="mt-2">
              <p className="text-2xl font-light text-cyan-400 tracking-tight">
                {stat.label.toLowerCase().includes('budget')
                  ...
                  : stat.label.toLowerCase().includes('taux') || stat.label.toLowerCase().includes('délais')
                  ? formatPercent(stat.value)
                  : stat.label.toLowerCase().includes('carbone')
                  ? formatUnit(stat.value, 'tCO₂')
                  : formatNumber(stat.value)}
              </p>
              <p className={`flex items-center text-sm mt-1 font-medium tracking-wide ${
                stat.trend === 'up' ? 'text-emerald-400' : 'text-rose-400'
              }`}>
                {stat.trend === 'up' ? '↑' : '↓'} {formatPercent(stat.change)}
              </p>
            </dd>
          </div>
        ))}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={performanceData}>
                ...
                <YAxis stroke="#6B7280" tickFormatter={(value) => formatPercent(value)} />
                <Tooltip formatter={(value) => formatPercent(value as number)} />
                <Line 
                  type="monotone" 
                  dataKey="actual" 
