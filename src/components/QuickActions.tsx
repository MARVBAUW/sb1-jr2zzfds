...
export function QuickActions() {
  ...
  const handleCreateProject = () => {
    // Determine which module we're in and redirect to the appropriate creation page
    if (location.pathname.startsWith('/maitre-oeuvre')) {
      navigate('/maitre-oeuvre/gestion-projets/saisie');
    } else if (location.pathname.startsWith('/maitre-ouvrage')) {
      navigate('/maitre-ouvrage/patrimoine/portefeuille/new');
    } else if (location.pathname.startsWith('/entreprise')) {
      navigate('/entreprise/supply-chain/commandes/new');
    } else if (location.pathname.startsWith('/particuliers')) {
      navigate('/particuliers/patrimoine/biens/new');
    } else if (location.pathname.startsWith('/agents')) {
      navigate('/agents/mandats/portefeuille/new');
    }
    setIsSearchOpen(false);
  };

  const handleProjectSelect = (projectId: string) => {
    // Determine which module we're in and redirect to the appropriate project page
    if (location.pathname.startsWith('/maitre-oeuvre')) {
      navigate(`/maitre-oeuvre/gestion-projets/fiche/${projectId}`);
    } else if (location.pathname.startsWith('/maitre-ouvrage')) {
      navigate(`/maitre-ouvrage/patrimoine/portefeuille/${projectId}`);
    } else if (location.pathname.startsWith('/entreprise')) {
      navigate(`/entreprise/supply-chain/commandes/${projectId}`);
    } else if (location.pathname.startsWith('/particuliers')) {
      navigate(`/particuliers/patrimoine/biens/${projectId}`);
    } else if (location.pathname.startsWith('/agents')) {
      navigate(`/agents/mandats/portefeuille/${projectId}`);
    }
    setIsSearchOpen(false);
  };
